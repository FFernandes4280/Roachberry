...
namespace WebApplicationApache.Controllers
{
    public class KafkaController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SendMessage(string content, string topico, string light = "pisca")
        {
            ...
            {
                jsonResponse.Add("content", content);
            }

            string jsonString = JsonConvert.SerializeObject(jsonResponse);
            await _kafkaProducer.ProduceAsync(topico, jsonString);
            return Json(true);
        }
